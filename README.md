...
| v1.5  | **配置化路由表（route_table_t）、Nginx/JSON/YAML 三格式配置、哈希表加速路由查找、405+Allow/404 处理、log_level/error_log 指令、location { } 路由块、启动安全校验** |
...
v1.5 将路由从硬编码的 if/else 链重构为可配置的路由表，支持三种配置文件格式，并优化了路由查找性能。

### 路由表架构

```
配置文件中 location / routes 定义
        │
        ▼
    route_table_t
  ┌─────────────────────────┐
  │ exact[]   ──→ 哈希表 O(1)  │  ← GET /hello, POST /users
  │ prefix[]  ──→ 线性扫描 O(m) │  ← GET /users/*, DELETE /users/*
  └─────────────────────────┘
        │
        ▼
  route_table_find(method, path)
        │
  ┌──────┼──────┐
  │      │      │
  ▼      ▼      ▼
...
每条路由是一个 `(method, path, match_type, handler)` 四元组。同一个路径的不同 HTTP 方法映射到不同的 handler，无需组合 handler。

### 路由匹配优先级

| 步骤 | 策略 | 复杂度 |
|------|------|--------|
| 1. 精确匹配 | djb2 哈希表查找 `"METHOD:/path"` | O(1) |
| 2. 前缀匹配 | 独立 `prefix[]` 数组线性扫描 | O(m), m≈5-10 |
| 3. 405 判定 | 路径存在但方法不匹配 → 返回 `Allow` 头 | — |
| 4. 404 判定 | 无任何路径匹配 | — |

- **精确路径优先于前缀路径**：`GET /users` 命中 exact 路由（用户列表），不会落到 `/users/*` 前缀路由（按名查找）
- **同一前缀内按插入顺序匹配**：长前缀（如 `/users/find-index/`）先插入，优先于短前缀（`/users/`）

### 配置文件三种格式

格式通过 `detect_config_format()` 自动检测，优先级 **Nginx > JSON > YAML**：

#### 1. Nginx 风格 (conf/server.conf)
...
```json
{
  ...
  "routes": [
    ...
    {"method": "GET",  "path": "/users",   "handler": "user_list"},
    {"method": "POST", "path": "/users",   "handler": "user_add"},
    {"method": "GET",  "path": "/users/*", "handler": "user_by_name"},
    {"method": "DELETE","path":"/users/*", "handler": "user_delete"}
  ]
}
```

#### 3. YAML 格式 (conf/server.yaml)
...
| 配置写法 | match_type | 匹配行为 |
|----------|------------|----------|
| `location = /path` 或 `"path":"/path"` | MATCH_EXACT | `strcmp` 精确匹配 |
| `location /path/*` 或 `"path":"/path/*"` | MATCH_PREFIX | `strncmp` 前缀匹配，`*` 之后的内容捕获到 `captured` 参数 |

### 启动安全校验

以下情况服务器**拒绝启动**（exit code ≠ 0）：

- **重复路由**：同一 `(method, path, match_type)` 出现多次
- **未知 handler**：handler 名称不在已知列表中
- **空 methods**：`location` 块缺少 `methods` 指令
...
| `hello` | GET → "Hello, Web!" |
| `help` | GET → API 帮助信息 |
| `sleep` | GET /sleep/{ms} → 延迟响应 |
| `index` | GET/HEAD / → 首页 index.html |
| `user_list` | GET /users → 列出所有用户 |
| `user_by_name` | GET /users/{name} → 按名查找 |
| `user_add` | POST /users → 添加用户 (CSV body) |
| `user_delete` | DELETE /users/{name} → 删除用户 |
| `user_find_index` | GET /users/find-index/{name} → BST 索引查找 |
| `user_compare` | GET /users/compare/{name} → 对比搜索性能 |
| `user_compare_verbose` | GET /users/compare-verbose/{name} → 详细对比 |
| `user_simple_find` | GET /user/{name} → 简单查找 |
| `search` | POST /search → 多条件 AND 搜索 |
| `delete_form` | POST /delete → 表单删除 |
| `blog` | GET/HEAD /blog/* → 博客静态文件 |
| `static` | GET/HEAD /* → 通用静态文件 |

### v1.5 新增配置指令

| 指令 | 格式 | 说明 |
|------|------|------|
| `log_level` | `log_level info` | 日志级别: debug / info / warning / error / none |
| `error_log` | `error_log logs/error.log warning` | 错误日志路径及过滤级别 |
| `location` | `location = /path { ... }` | 精确路由块 |
| `location` | `location /path/* { ... }` | 前缀/通配路由块 |
| `handler` | `handler <name>;` | 指定 C handler 函数 |
| `methods` | `methods GET POST;` | 允许的 HTTP 方法列表 |

### 向后兼容

如果配置文件中**没有定义任何 `location` 块**，服务器自动使用内置默认路由表，与 v1.4 行为完全一致。旧版配置文件无需修改即可使用。

## 配置文件格式 (conf/server.conf)

支持三种格式（Nginx / JSON / YAML），自动检测：
...
bash tests/test_day13.sh   # POST /search 状态码测试（400/413/415，v1.4）
bash tests/test_day15.sh   # 路由表功能测试（精确/前缀匹配、405/404、启动校验，v1.5）
```
